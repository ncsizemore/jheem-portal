...
- **Register: state the thesis once, then go dry.** The H1 carries the claim (declarative, calibrated: "is projected to cost more than it saves"); every other line presents. No vivid diction doing the numbers' job (backfire, illusion, watch it flip), and a title may not claim more than its own footnote allows (a "predicts" title over a "descriptive, no fitted line" footnote is a calibration bug).
- **Exhibit virtues; never claim them.** "This estimate is deliberately conservative" became "Excluded from this estimate: … Each exclusion lowers the estimate." The reader supplies the adjective. Same for rigor, honesty, restraint.
- **The page never talks about itself.** No design rationale in captions ("kept out of the interface on purpose"), no references to its own sections ("the illusion from the hero"), no audience tags ("for reviewers"), no internal workflow ("flagged for review"). Seniority is also choosing what not to explain: deleted explanations beat dried ones.
